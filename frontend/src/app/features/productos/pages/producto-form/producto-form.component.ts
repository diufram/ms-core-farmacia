      ...
      <form *ngIf="!loadingData()" [formGroup]="form" (ngSubmit)="guardar()" class="space-y-4">
        <p-card>
          <ng-template pTemplate="header">
            <div class="px-6 pt-5">
              <h3 class="text-lg font-semibold text-color m-0">Datos del producto</h3>
            </div>
          </ng-template>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            <div>
              <p-floatlabel variant="on">
                <input pInputText id="codigo" type="text" formControlName="codigo" class="w-full" />
                <label for="codigo">Código *</label>
              </p-floatlabel>
              <small *ngIf="invalid('codigo')" class="text-red-500 mt-1 block">
            ...
            <div>
              ...
              <small *ngIf="invalid('precio_venta')" class="text-red-500 mt-1 block">
            ...
            <div class="md:col-span-2">
              ...
              <small *ngIf="invalid('categoriaId')" class="text-red-500 mt-1 block">
                La categoría es requerida
              </small>
              <small *ngIf="categorias().length === 0" class="text-muted-color mt-1 block">
                No hay categorías disponibles en tu sucursal. Crea una primero.
              </small>
            </div>
          </div>
        </p-card>
...
export class ProductoFormComponent implements OnInit {
  ...
  private cargarCategorias(): void {
    ...
    const sucursalId = user?.rol === 'super_admin' ? null : user?.sucursal_id;

    this.categoriasService.list(sucursalId ?? undefined).subscribe({
  ...
  guardar(): void {
    ...
    if (this.esEdicion() && this.productoId !== null) {
      ...
      this.productosService.update(this.productoId, input).subscribe({
        next: () => {
