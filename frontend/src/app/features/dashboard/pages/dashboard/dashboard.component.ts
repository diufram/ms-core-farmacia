  ...
  template: `
    <div class="grid grid-cols-12 gap-4">
      ...
      <div *ngIf="loading()" class="col-span-12 flex justify-center py-12">
        <p-progress-spinner styleClass="w-4 h-4" strokeWidth="3" />
      </div>
      ...
      <ng-container *ngIf="kpis() as data">
        ...
        >
          <p-card styleClass="h-full">
            <div class="flex items-center justify-between">
              <div>
                <span class="text-muted-color text-sm"> Total ventas </span>
        ...
        >
          <p-card styleClass="h-full">
            <div class="flex items-center justify-between">
              <div>
                <span class="text-muted-color text-sm"> Productos </span>
                <div class="text-2xl font-semibold text-color mt-2">
                  {{ data.totalProductos }}
                </div>
                <div class="text-muted-color text-xs mt-1">
        ...
        <div *ngIf="esSuperAdmin()" class="col-span-12 sm:col-span-6 lg:col-span-4">
          <p-card styleClass="h-full">
            <div class="flex items-center justify-between">
              <div>
                <span class="text-muted-color text-sm"> Sucursales </span>
                <div class="text-2xl font-semibold text-color mt-2">
                  {{ data.totalSucursales }}
                </div>
                <div class="text-muted-color text-xs mt-1">Activas</div>
              </div>
              <div
        ...
        <div [class]="esSuperAdmin() ? 'col-span-12 lg:col-span-8' : 'col-span-12'">
          <p-card>
            <ng-template pTemplate="header">
              <div class="px-6 pt-5 pb-2">
                <h3 class="text-lg font-semibold text-color m-0">Ventas últimos 30 días</h3>
              </div>
            </ng-template>
            <p-chart
              type="line"
        ...
        <div *ngIf="esSuperAdmin()" class="col-span-12 lg:col-span-4">
          <p-card>
            <ng-template pTemplate="header">
              <div class="px-6 pt-5 pb-2">
                <h3 class="text-lg font-semibold text-color m-0">Ventas por sucursal</h3>
              </div>
            </ng-template>
            <p-chart
              type="bar"
        ...
        <div class="col-span-12 lg:col-span-6">
          <p-card>
            <ng-template pTemplate="header">
              <div class="px-6 pt-5 pb-2">
                <h3 class="text-lg font-semibold text-color m-0">Top productos vendidos</h3>
              </div>
            </ng-template>
            <p-table
            ...
            >
              ...
              <ng-template pTemplate="emptymessage">
                <tr>
                  <td colspan="4" class="text-center text-muted-color py-4">
          ...
          <p-card>
            <ng-template pTemplate="header">
              <div class="px-6 pt-5 pb-2 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-color m-0 flex items-center gap-2">
                  <i class="pi pi-users text-primary"></i>
                  Clientes frecuentes
                  <p-tag value="Score ML" severity="info" [rounded]="true" />
                </h3>
              </div>
            </ng-template>
            <p-table
              ...
              <ng-template pTemplate="body" let-cli>
                <tr>
                  ...
                  <td class="text-right text-sm font-medium">
                    Bs
                    {{ cli.total_comprado | number: '1.2-2' }}
                  </td>
                </tr>
              </ng-template>
              <ng-template pTemplate="emptymessage">
                <tr>
                  <td colspan="3" class="text-center text-muted-color py-4 text-sm">
                    Sin clientes frecuentes en el período.
                  </td>
                </tr>
              </ng-template>
            </p-table>
            ...
            <div class="px-2 pb-2">
              <div class="flex items-baseline gap-2 mb-3">
                <span
                  ...
                  [ngClass]="tendenciaTextClasses(t.crecimiento_pct)"
                >
                  {{ t.crecimiento_pct >= 0 ? '+' : '' }}{{ t.crecimiento_pct | number: '1.1-1' }}%
                </span>
                <i
                  class="pi text-xl"
                  [ngClass]="
                  ...
                  "
                ></i>
                <span class="text-muted-color text-xs ml-2"> vs período anterior </span>
              </div>
              <div class="grid grid-cols-2 gap-3 text-sm mb-3">
                <div class="bg-surface-50 dark:bg-surface-800 rounded p-3">
                  <div class="text-muted-color text-xs mb-1">Período actual</div>
                  <div class="font-semibold">
                    Bs
                    {{ t.ventas_actual | number: '1.2-2' }}
                  </div>
                  <div class="text-muted-color text-xs">{{ t.cantidad_actual }} ops</div>
                </div>
                <div class="bg-surface-50 dark:bg-surface-800 rounded p-3">
                  <div class="text-muted-color text-xs mb-1">Período anterior</div>
                  <div class="font-semibold">
                    Bs
                    {{ t.ventas_anterior | number: '1.2-2' }}
                  </div>
                  <div class="text-muted-color text-xs">{{ t.cantidad_anterior }} ops</div>
                </div>
              </div>
              <div
                class="flex items-center justify-between text-sm pt-2 border-t border-surface-200 dark:border-surface-700"
              >
                <div *ngIf="t.mejor_dia" class="flex items-center gap-2">
                  <i class="pi pi-star-fill text-yellow-500"></i>
                  <span class="text-muted-color text-xs"> Mejor día: </span>
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-muted-color text-xs"> Anomalías: </span>
                  <p-tag
                    [value]="t.dias_anomalos + ' día(s)'"
                    [severity]="t.dias_anomalos > 0 ? 'warn' : 'success'"
                    [rounded]="true"
                  />
                </div>
              </div>
                ...
                <tr>
                  <td>
                    ...
                    <div class="text-muted-color text-xs">
                      {{ cat.productos_stock_bajo }}/{{ cat.total_productos }}
                      stock bajo ·
                      {{ cat.ventas_periodo }}
                      vend.
                    </div>
                    ...
                    <div class="inline-flex items-center gap-2">
                      ...
                      >
                        <div
                          ...
                          [style.width.%]="cat.score_riesgo * 100"
                          [ngClass]="barraFillClassesRiesgo(cat.score_riesgo)"
                        ></div>
                      </div>
                      <span class="text-xs font-medium">
                        {{ cat.score_riesgo * 100 | number: '1.0-0' }}%
                      </span>
                    </div>
                  </td>
                  <td>
                      ...
                      [value]="
                        cat.score_riesgo >= 0.7
                          ...
                          : cat.score_riesgo >= 0.4
                            ...
                            : 'BAJO'
                      "
                      [severity]="riesgoColor(cat.score_riesgo)"
                      [rounded]="true"
                    />
                  </td>
                </tr>
            ...
            <ng-template pTemplate="header">
              <div class="px-6 pt-5 pb-2 flex items-center justify-between">
                <div>
                  ...
                  <p class="text-muted-color text-xs m-0 mt-1">
                    Top 5 productos con mayor probabilidad de agotarse ({{ rangoSeleccionado }}
                    días)
                  </p>
                </div>
              </div>
            ...
            >
              <ng-template pTemplate="header">
                <tr>
                  ...
                  <th class="text-right">Días p/ agotar</th>
                  <th class="text-right">Score</th>
                  <th>Riesgo</th>
                </tr>
              </ng-template>
                ...
                <tr>
                  ...
                  <td class="text-right">
                    <div
                      ...
                      [pTooltip]="tooltipFeatures(pred.features)"
                    ...
                    >
                      ...
                      >
                        <div
                          class="h-full transition-all"
                          [style.width.%]="prediccionPorcentaje(pred.score)"
                          [ngClass]="barraFillClasses(pred.nivel_riesgo)"
                        ></div>
                      </div>
                      <span class="text-xs font-medium">
                        {{ prediccionPorcentaje(pred.score) }}%
                      </span>
                    </div>
                  </td>
                  <td>
                    <p-tag
                      ...
                      [severity]="prediccionColor(pred.nivel_riesgo)"
                      [rounded]="true"
                    />
                  </td>
                </tr>
              ...
              <ng-template pTemplate="emptymessage">
                <tr>
                  <td colspan="7" class="text-center text-muted-color py-4">
                    Sin datos suficientes para predecir demanda.
                  </td>
                </tr>
              </ng-template>
            </p-table>
        ...
        <div class="col-span-12 lg:col-span-7">
          <p-card>
            ...
            >
              ...
              <ng-template pTemplate="emptymessage">
                <tr>
                  <td colspan="5" class="text-center text-muted-color py-4 text-sm">
        ...
        <div class="col-span-12 lg:col-span-5">
          <p-card>
            <ng-template pTemplate="header">
              <div class="px-6 pt-5 pb-2 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-color m-0">Stock bajo</h3>
                <p-tag [value]="data.productosStockBajo + ' alertas'" severity="danger" />
              </div>
            </ng-template>
            <p-table
            ...
            >
                ...
                <tr>
                  ...
                  <td class="text-right">
                    <p-tag
                      ...
                      [severity]="prod.stock_actual <= 0 ? 'danger' : 'warn'"
                    />
                  </td>
                </tr>
              </ng-template>
              <ng-template pTemplate="emptymessage">
                <tr>
                  <td colspan="3" class="text-center text-muted-color py-4">
...
export class DashboardComponent implements OnInit {
  ...
  private initChartOptions(): void {
    ...
    const textColorSecondary = this.getCssVar('--text-color-secondary') || '#64748b';
    const surfaceBorder = this.getCssVar('--surface-border') || '#e2e8f0';
      ...
      datasets: [
        {
          ...
          fill: true,
          borderColor: this.getCssVar('--primary-color') || '#3b82f6',
          backgroundColor: 'rgba(59, 130, 246, 0.12)',
          tension: 0.35,
          pointBackgroundColor: this.getCssVar('--primary-color') || '#3b82f6',
          pointRadius: 3,
          pointHoverRadius: 5
        }
      ]
  ...
  cargar(): void {
    ...
    const sucursalId =
      this.esSuperAdmin() && this.filtroSucursalId != null ? Number(this.filtroSucursalId) : null;
    ...
    this.dashboardService
      ...
      .subscribe({
        next: (data) => {
          this.kpis.set(data);
          this.prediccionesDemanda.set(this.calcularPrediccionesDemanda(data));
  ...
  private calcularPrediccionesDemanda(data: DashboardKpis): PrediccionDemanda[] {
    ...
    for (const top of data.topProductos) {
      ventasPorProducto.set(
        top.producto_id,
        (ventasPorProducto.get(top.producto_id) ?? 0) + top.cantidad_vendida
      );
    }
    ...
    const meanVendido = ventasValues.reduce((a, b) => a + b, 0) / ventasValues.length;
    const stdVendido = Math.max(
      1,
      Math.sqrt(
        ventasValues.reduce((acc, v) => acc + (v - meanVendido) ** 2, 0) / ventasValues.length
      )
    );
    ...
    for (const top of data.topProductos) {
      ...
      const stockActual = stockInfo ? stockInfo.stock_actual : STOCK_REFERENCIA;
      const cantidadVendida = top.cantidad_vendida;
      const enStockBajo = !!stockInfo;

      const velocidadDiaria = cantidadVendida / RANGO_DIAS;
      const ratioPresion = velocidadDiaria > 0 ? Math.min(velocidadDiaria / stockActual, 5) : 0;
      const factorDemanda = Math.max(0, (cantidadVendida - meanVendido) / stdVendido);
      const factorStock = 1 - Math.min(stockActual / STOCK_REFERENCIA, 1);

      const score =
        0.45 * Math.tanh(ratioPresion) +
        0.3 * Math.min(factorDemanda / 2, 1) +
        0.2 * factorStock +
        0.05 * (enStockBajo ? 1 : 0);

      const diasEstimados = velocidadDiaria > 0 ? Math.round(stockActual / velocidadDiaria) : 9999;
  ...
  private calcularTendencia(data: DashboardKpis): Tendencia {
    ...
    const ventasAnterior = primeraMitad.reduce((acc, d) => acc + d.total, 0);
    const cantidadAnterior = primeraMitad.reduce((acc, d) => acc + d.cantidad, 0);
    ...
    const mejorDia = data.ventasPorDia.reduce<{ fecha: string; total: number } | null>((acc, d) => {
    ...
    const mean = totales.reduce((a, b) => a + b, 0) / Math.max(1, totales.length);
    const std = Math.max(
      1,
      Math.sqrt(totales.reduce((acc, v) => acc + (v - mean) ** 2, 0) / Math.max(1, totales.length))
    );
  ...
  barraFillClasses(nivel: PrediccionDemanda['nivel_riesgo']): { [key: string]: boolean } {
    return {
  ...
  tendenciaTextClasses(pct: number): { [key: string]: boolean } {
    ...
    return {
      ...
      'text-surface-500 dark:text-surface-400': nivel === 'secondary'
    };
  }
}
