      ...
      <form *ngIf="!loadingData()" [formGroup]="form" (ngSubmit)="guardar()" class="space-y-4">
        <p-card>
          <ng-template pTemplate="header">
            <div class="px-6 pt-5">
              <h3 class="text-lg font-semibold text-color m-0">Datos personales</h3>
            </div>
          </ng-template>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
            ...
            <div>
              ...
              <small *ngIf="invalid('apellido')" class="text-red-500 mt-1 block">
        ...
        <p-card>
          <ng-template pTemplate="header">
            <div class="px-6 pt-5">
              <h3 class="text-lg font-semibold text-color m-0">Datos de cuenta</h3>
            </div>
          </ng-template>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            <div>
              ...
              <small *ngIf="invalid('nombre_usuario')" class="text-red-500 mt-1 block">
                Mínimo 3 caracteres
              </small>
            </div>
            ...
            <div>
              ...
              <small *ngIf="invalid('correo_electronico')" class="text-red-500 mt-1 block">
            ...
            <div *ngIf="!esEdicion()">
              ...
              <small *ngIf="invalid('contrasena')" class="text-red-500 mt-1 block">
                Mínimo 8 caracteres
              </small>
            </div>
        ...
        <p-card *ngIf="!esEdicion()">
          <ng-template pTemplate="header">
            <div class="px-6 pt-5">
              <h3 class="text-lg font-semibold text-color m-0">Sucursales iniciales</h3>
              <p class="text-muted-color text-sm m-0 mt-1">
                Asigna al usuario a una o más sucursales. Podrás administrarlas más tarde.
              </p>
            </div>
          </ng-template>
          ...
          <div class="space-y-3 pt-2">
            <div
              *ngFor="let ctrl of sucursalesControls.controls; let i = index"
        ...
        <p-card *ngIf="esEdicion()">
          <ng-template pTemplate="header">
            <div class="px-6 pt-5 flex items-center justify-between">
              <div>
                <h3 class="text-lg font-semibold text-color m-0">Sucursales asignadas</h3>
                <p class="text-muted-color text-sm m-0 mt-1">
                  Gestiona las sucursales a las que pertenece este usuario.
                </p>
              </div>
            </div>
          </ng-template>
          ...
          <div class="space-y-3 pt-2">
            <p-table [value]="usuario()?.asignaciones ?? []" styleClass="p-datatable-sm">
              <ng-template pTemplate="header">
                <tr>
                  <th>Sucursal</th>
                  <th>Estado</th>
              ...
              <ng-template pTemplate="body" let-asig>
                <tr>
                  ...
                  <td>
                    <p-tag
                      [value]="asig.activo ? 'Activo' : 'Inactivo'"
                      [severity]="asig.activo ? 'success' : 'danger'"
                    />
                  </td>
                  <td class="text-right">
                    <p-button
              ...
              <ng-template pTemplate="emptymessage">
                <tr>
                  <td colspan="3" class="text-center text-muted-color py-4">
                    El usuario no está asignado a ninguna sucursal.
                  </td>
                </tr>
              </ng-template>
            </p-table>
              ...
              <div class="flex-1 min-w-[200px]">
                <p-floatlabel variant="on">
                  <p-select
                    ...
                    filterBy="nombre"
                  />
                  <label for="nueva-sucursal">Agregar a sucursal</label>
                </p-floatlabel>
              </div>
              <p-button
                icon="pi pi-plus"
...
export class UsuarioFormComponent implements OnInit {
  ...
  sucursalesDisponibles(): Sucursal[] {
    const asignadas = new Set(
      this.sucursalesIniciales.map((g) => g.get('sucursalId')?.value).filter((v) => v != null)
    );
  ...
  sucursalesParaAsignar(): Sucursal[] {
    const asignadasIds = new Set((this.usuario()?.asignaciones ?? []).map((a) => a.sucursal.id));
  ...
  guardar(): void {
    ...
    if (this.esEdicion() && this.usuarioId !== null) {
      ...
      this.usuariosService.update(this.usuarioId, input).subscribe({
        next: (res) => {
  ...
  asignarSucursal(): void {
    ...
    this.usuariosService.assignSucursal(this.usuarioId, input).subscribe({
      next: (res) => {
        this.toast.success(res.message);
  ...
  quitarAsignacion(asig: UsuarioAsignacion): void {
    ...
    this.confirmation.confirm({
      ...
      accept: () => {
        this.usuariosService.unassignSucursal(this.usuarioId!, asig.sucursal.id).subscribe({
          next: (res) => {
            this.toast.success(res.message);
            this.usuario.set(res.usuario);
          },
