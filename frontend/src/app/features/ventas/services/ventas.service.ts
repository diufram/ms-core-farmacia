...
export class VentasService {
  ...
  create(input: CreateVentaInput): Observable<VentaPayload> {
    return this.apollo
      .mutate<{ createVenta: VentaPayload }, { input: CreateVentaInput }>({
  ...
  cambiarEstado(id: number, nuevoEstado: EstadoVenta): Observable<VentaPayload> {
    return this.apollo
      .mutate<{ cambiarEstadoVenta: VentaPayload }, { id: number; nuevoEstado: string }>({
  ...
  delete(id: number): Observable<{ message: string }> {
    return this.apollo
      .mutate<{ deleteVenta: { message: string } }, { id: number }>({
  ...
  verificarIntegridad(
    id: number
  ): Observable<{ isVerified: boolean; currentHash: string; blockchainHash: string }> {
    return this.apollo
      .query<any>({
      ...
      .pipe(map((res) => res.data.verificarIntegridadVenta));
  }
}
