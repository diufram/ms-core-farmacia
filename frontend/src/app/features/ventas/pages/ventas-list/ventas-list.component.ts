...
@Component({
  ...
  template: `
    <div class="card">
      ...
      <p-dialog
        header="Verificación de Blockchain"
      ...
      >
        ...
        <div *ngIf="!isVerifying() && verificationResult()" class="py-3">
          <div *ngIf="verificationResult()?.isVerified" class="flex flex-col items-center">
            ...
            <p class="text-center text-color-secondary m-0 mb-4">
              Los datos de esta venta coinciden exactamente con el registro inmutable en la
              blockchain.
            </p>
          ...
          <div *ngIf="!verificationResult()?.isVerified" class="flex flex-col items-center">
            ...
            <p class="text-center text-color-secondary m-0 mb-4">
              Los datos de esta venta en la base de datos han sido alterados y ya no coinciden con
              el registro original de la blockchain.
            </p>

            <div
              class="w-full bg-red-50 dark:bg-red-900/20 p-3 rounded-lg text-xs break-all border border-red-200 dark:border-red-800"
            >
              <span class="font-bold text-red-700 dark:text-red-400 block mb-1"
                >Hash Original (Blockchain):</span
              >
              <span class="text-red-600 dark:text-red-300 block mb-3">{{
                verificationResult()?.blockchainHash
              }}</span>

              <span class="font-bold text-red-700 dark:text-red-400 block mb-1"
                >Hash Alterado (Base de Datos):</span
              >
...
export class VentasListComponent implements OnInit {
  ...
  private opcionesEstadoParaFila(venta: Venta): SelectButtonOption[] {
    ...
    const todas: EstadoVenta[] = ['PENDIENTE', 'PREPARADA', 'CONFIRMADA', 'RECHAZADA'];
  ...
  private verDetalle(v: Venta): void {
    const detalles = v.detalles
      .map((d) => `${d.producto_nombre} x${d.cantidad} = Bs ${d.subtotal}`)
      .join('\n');
