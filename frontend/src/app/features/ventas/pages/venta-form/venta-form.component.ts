  ...
  template: `
    <div class="max-w-4xl mx-auto">
      <div class="flex items-center gap-3 mb-4">
        ...
        />
        <div>
          <h1 class="text-2xl font-semibold text-color m-0">Nueva Venta</h1>
        </div>
      </div>
      ...
      <form [formGroup]="form" (ngSubmit)="guardar()" class="space-y-4">
        <p-card>
          <ng-template pTemplate="header">
            <div class="px-6 pt-5 flex justify-between items-center">
              <h3 class="text-lg font-semibold text-color m-0">Datos generales</h3>
              <p-tag
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            <div *ngIf="esSuperAdmin()">
              ...
              />
              <small *ngIf="invalid('sucursalId')" class="text-red-500 mt-1 block">
            ...
            <div *ngIf="!esSuperAdmin() && !esCliente()">
              ...
              <input pInputText type="text" [value]="sucursalNombre()" disabled class="w-full" />
            </div>
        ...
        <p-card *ngIf="esEmpleado()">
          <ng-template pTemplate="header">
            <div class="px-6 pt-5">
              <h3 class="text-lg font-semibold text-color m-0">Cliente walk-in (opcional)</h3>
          ...
          <div
            *ngIf="form.get('cliente_walk_in')?.value"
          ...
          >
            <div>
              ...
              />
              <small *ngIf="invalid('cliente_nombre')" class="text-red-500 mt-1 block">
                El nombre es obligatorio para walk-in
              </small>
            </div>
            <div>
              <label class="field-label">Celular</label>
              <input pInputText type="text" formControlName="cliente_celular" class="w-full" />
            </div>
            <div>
        ...
        <p-card>
          <ng-template pTemplate="header">
            <div class="px-6 pt-5 flex justify-between items-center">
              <h3 class="text-lg font-semibold text-color m-0">Productos</h3>
              <p-button
                icon="pi pi-plus"
          ...
          <div *ngIf="detalles.length === 0" class="text-center py-10 text-muted-color">
            <i class="pi pi-shopping-cart text-5xl mb-4 block opacity-60"></i>
            <p class="text-base">Agrega al menos un producto para registrar la venta.</p>
          </div>
            ...
            >
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-4 items-start">
                ...
                <div class="sm:col-span-1 lg:col-span-2 min-w-0">
                  ...
                  <p-inputNumber formControlName="cantidad" [min]="1" styleClass="w-full" />
                </div>
...
export class VentaFormComponent implements OnInit {
  ...
  cargarProductos(): void {
    ...
    this.productosService.list({ sucursalId, categoriaId: null }).subscribe({
  ...
  subtotalLinea(index: number): number {
    ...
    const precio = Number(linea.get('precioUnitario')?.value) || this.getPrecioProducto(index);
  ...
  guardar(): void {
    ...
    const detalles: VentaDetalleInput[] = (v.detalles as DetalleLinea[]).map((d) => ({
