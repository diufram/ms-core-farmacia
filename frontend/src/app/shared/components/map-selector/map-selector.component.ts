...
export class MapSelectorComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private initializeMap(): void {
    try {
      ...
      const centerLat = this.hasValidCoordinates() ? this.value.lat : this.defaultLocation.lat;
      const centerLng = this.hasValidCoordinates() ? this.value.lng : this.defaultLocation.lng;
  ...
  private scheduleAddressResolution(lat: number, lng: number, delayMs = 220): void {
  ...
  private async resolveAddressFromCoordinates(lat: number, lng: number): Promise<void> {
    ...
    try {
      ...
      const resolvedAddress = data?.address?.trim() || 'Punto seleccionado';
