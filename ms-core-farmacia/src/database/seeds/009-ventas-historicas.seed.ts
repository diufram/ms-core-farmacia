...
function generarVenta(
  ...
  while (productosElegidos.length < cantidadProductos && usados.size < productos.length) {
  ...
  venta.cliente_celular = esWalkIn ? `7${randomInt(1000000, 9999999)}` : null;
...
export const ventasHistoricasSeed: Seed = {
  ...
  run: async () => {
    ...
    if (!superAdmin) {
      console.log('- Super admin no existe, seed de ventas históricas omitido.');
      return;
    }
    ...
    for (const { slug, codigo: sucCode } of SUCURSALES) {
      ...
      if (productos.length === 0) {
        console.log(`- Sin productos en ${slug}, seed omitido.`);
        continue;
      }
      ...
      for (let diasAtras = 90; diasAtras >= 1; diasAtras--) {
        ...
        const ventasHoy = Math.max(1, Math.round(baseVentasPorDia * factorFinDeSemana));
        ...
        for (let i = 0; i < ventasHoy; i++) {
          ...
          await seedDataSource.transaction(async (manager) => {
            const ventaGuardada = await manager.save(manager.create(Venta, venta));
            for (const det of detalles) {
              det.venta = ventaGuardada;
              await manager.save(manager.create(VentaDetalle, det));
              if (estado === 'CONFIRMADA') {
                const prod = productos.find((p) => p.id === det.producto.id);
                if (prod) {
                  prod.stock_actual = Math.max(0, prod.stock_actual - det.cantidad);
      ...
      console.log(`- ${slug}: ${contador} ventas históricas (90 días).`);
    }

    console.log(`- Total: ${totalCreadas[0]} ventas históricas creadas.`);
  },
};
